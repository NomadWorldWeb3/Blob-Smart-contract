...
contract BLOB is ERC20, Ownable {
    ...
    {   
        ...
        _mintOnce(owner(), 1 * 10**11 * (10 ** decimals()));
    ...
    function afterStartBlob() external onlyOwner {
        feeOnBuy = 0;
        feeOnSell = 0;
        feeOnTransfer = 0;
    ...
    event JOEINGSTART(bool JOEING);

    bool public JOEING;

    function startBlob() external onlyOwner{
        require(!JOEING, "CSLT: Trading already enabled.");
        JOEING = true;
        swapEnabled = true;

        emit JOEINGSTART(JOEING);
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        require(JOEING || _isExcludedFromFees[from] || _isExcludedFromFees[to], "CSLT: Trading not yet enabled!");
